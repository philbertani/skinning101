//https://twgljs.org/examples/gpgpu-particles.html  copied from here

"use strict";

//this one does NOT use the new webgl2 transformFeedback functions of vertex shaders 
...
const fPhysics = `
    ...
    void main() {
      ...
      x = particle.xy;
      v = particle.zw;
      r = x - gravity_center*mat2(cos(an),sin(an),-sin(an),cos(an));
      d = x - v_texcoord;

      // physics emulation
      a = (1.-off_gravity)*-normalize(r)*dt; //(1.0 - off_gravity) * ( -normalize(r) * dt * clamp(3. / dot(r, r), 0., 2.) ) ; //+ 0.03 * (n2rand()*.2 - .2) );
...
const vDraw = `
    ...
    void main () {
      ...
      gl_PointSize = 10.; //2.*abs(max(particle.x-.5,particle.y-.5));
...
//this touch stuff is instructive
